...
class PostAdminModel extends BaseModel
{
    ...
    public function addPost()
    {
        ...
        $data['visibility'] = 1;
        if ($this->generalSettings->approve_posts_before_publishing == 1 && user()->role != 'admin') {
            $data['visibility'] = 0;
        }
    ...
    public function setData()
    {
        ...
        if (!empty($data["title_slug"])) {
            $data["title_slug"] = removeSpecialCharacters(trim($data["title_slug"]), true);
        } else {
            $data["title_slug"] = strSlug(trim($data["title"]));
        }
