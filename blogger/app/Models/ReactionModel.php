...
class ReactionModel extends BaseModel
{
    ...
    public function saveReaction($postId, $reaction)
    {
        if (in_array($reaction, $this->arrayReactions)) {
            if (isReactionVoted($postId, $reaction)) {
                $this->session->set('inf_reaction_' . $reaction . '_' . $postId, '0');
                setCookie('inf_reaction_' . $reaction . '_' . $postId, '0');
                $this->decreaseReactionVote($postId, $reaction);
                $this->decreasePostVoteSession($postId);
            } else {
                $this->session->set('inf_reaction_' . $reaction . '_' . $postId, '1');
                setCookie('inf_reaction_' . $reaction . '_' . $postId, '1');
    ...
    {
        $key = 'inf_reaction_vote_count_' . $postId;
        $count = $this->session->get($key);
        if (empty($count)) {
            $count = 0;
        }
        $this->session->set($key, $count + 1);
        setCookie($key, $count + 1);
    }
    ...
    public function setVotedReactionsSession($postId)
    {
        //vote count
        $key = 'inf_reaction_vote_count_' . $postId;
        if (!empty(getCookie($key))) {
            $this->session->set($key, getCookie($key));
        } else {
            $this->session->set($key, 0);
        }
        //like
        $key = 'inf_reaction_like_' . $postId;
        $this->setReactionSession($key);
        //dislike
        $key = 'inf_reaction_dislike_' . $postId;
        $this->setReactionSession($key);
        //love
        $key = 'inf_reaction_love_' . $postId;
        $this->setReactionSession($key);
        //funny
        $key = 'inf_reaction_funny_' . $postId;
        $this->setReactionSession($key);
        //angry
        $key = 'inf_reaction_angry_' . $postId;
        $this->setReactionSession($key);
        //sad
        $key = 'inf_reaction_sad_' . $postId;
        $this->setReactionSession($key);
        //wow
        $key = 'inf_reaction_wow_' . $postId;
        $this->setReactionSession($key);
    }
    ...
    {
        if (!empty(getCookie($key)) && getCookie($key) == '1') {
