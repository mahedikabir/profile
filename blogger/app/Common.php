...
if (!function_exists('isReactionVoted')) {
    function isReactionVoted($postId, $reaction)
    {
        ...
        if (!empty($session->get("inf_reaction_" . $reaction . "_" . $postId)) && $session->get("inf_reaction_" . $reaction . "_" . $postId) == '1') {
            return true;
        }
        return false;
    }
...
//prevent author
if (!function_exists('preventAuthor')) {
    function preventAuthor()
    {
        if (!isAdmin()) {
...
if (!function_exists('checkNewsletterModal')) {
    function checkNewsletterModal()
    {
        if (!authCheck() && Globals::$generalSettings->newsletter_status == 1 && Globals::$generalSettings->newsletter_popup == 1) {
            if (empty(getCookie("inf_news_mdl"))) {
                setCookie('inf_news_mdl', '1');
                return true;
            }
        }
        return false;
